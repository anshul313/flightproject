...
var development_database_url = 'https://data.ailment92.hasura-app.io/';
...
var development_authToken = 'Bearer 2l29aj9qcmfuys78eskup7epa3e45bfh';
...
function airport_user_profile_function(req, res, next) {
  ...
  var startTime;
  var endTime;
  var startMiliTime = new Date().getTime();
  var endMiliTime = startMiliTime + (3600000 * 2);
  ...
  find(checkData, url, res, function(err, userData) {
    ...
    if (userData.length > 0) {
      startTime = userData[0].entry_time - (3600000 * 2);
      endTime = userData[0].entry_time + (3600000 * 2);
      var checkData = {
        "columns": ["*"],
        where: {
          airport_id: userData[0].airport_id,
          "$and": [{
            entry_time: {
              "$gte": startTime
            }
